...
class BrewingRecipeSerializer : RecipeSerializer<BrewingRecipe> {
    ...
    override fun streamCodec(): StreamCodec<RegistryFriendlyByteBuf, BrewingRecipe> = STREAM_CODEC
    ...
    companion object {
        ...
        val STREAM_CODEC: StreamCodec<RegistryFriendlyByteBuf, BrewingRecipe> = StreamCodec.composite(
            ByteBufCodecs.collection({ mutableListOf() }, Ingredient.CONTENTS_STREAM_CODEC), BrewingRecipe::ingredients,
            ByteBufCodecs.STRING_UTF8, { it.fireType.name },
            PotionRef.Companion.STREAM_CODEC, BrewingRecipe::result,
            { items, fireType, result ->
                BrewingRecipe(
                    items,
                    FireType.valueOf(fireType),
                    result
